#ifndef __AGENT_IP_SCAN_FILTER_H__
#define __AGENT_IP_SCAN_FILTER_H__
...
class IPScanFilter : public FlowFilter {
 ...
 public:
  ...
  static IPScanFilter* Create(u32 dev_id, const string& model,
                               DBBuilder* builder, DBBuilder* event_builder);

  ~IPScanFilter() override {}
  bool CheckFlow(FlowPtr flow) override;
  static event::GenEventRes UpdateFinished(std::vector<master_record_t>* flowset, unique_ptr<IPScanFilter>& ptr, const string& model);
 ...
 private:
  ...
  IPScanFilter(u32 dev_id, const string& model);
  ...
  void InsertScanToTSDB(const IPScanKey&, const IPScanStat&);
  void CheckScan(const Slice& key, const Slice& val, const feature::FeatureReq& req);
  void AddRecord(const IPScanKey& key, const feature::FeatureRecord& new_rec);
  void GenerateEvents(const IPScanKey&, const IPScanStat&, event::GenEventRes* events);
	void DividePatterns(Pattern& pat, vector<string>& res);
  void Generate(const IPScanKey& s, const IPScanStat& p, event::GenEventRes* events, Pattern& pat);
	void DivideMode();
	void GenerateFeature(const IPScanKey& s, const IPScanStat& p);
	Match_res* Pickout(const IPScanKey& s, const IPScanStat& p,
  ...
  void GenEventFeature(const IPScanKey& s, const event::GenEventRecord* e);
  ...
  std::set<Conv> convs_;
  std::map<IPScanKey, IPScanStat> scans_;
  std::map<IPScanKey, feature::FeatureResponse> res_ipscan_;
  EventGenerators event_generators_;
  std::vector<event::GenEventRecord*> events_;
  std::map<u32, std::set<IPScanKey>> caches_;
  std::map<IPScanKey, std::map<EventKey, EventValue> > event_details_;

	std::set<Pattern> ip_peerip_proto_;
  std::set<Pattern> ip_peerip_;
  std::set<Pattern> ip_proto_;
  std::set<Pattern> peerip_proto_;
  std::set<Pattern> ip_;
  std::set<Pattern> peerip_;
  std::set<Pattern> proto_;
  std::set<Pattern> peers_;
};

#endif // __AGENT_IP_SCAN_FILTER_H__
